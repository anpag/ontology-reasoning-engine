...
A highly scalable, memory-safe microservice designed to ingest massive Semantic Web Ontologies (OWL, Turtle, JSON-LD), apply rigorous Description Logic materialization via Forward Chaining, and export the resulting Knowledge Graph into BigQuery.
...
3. **Reasoning Engine (Native Rust)**: 
   A custom-compiled Rust binary handles the mathematical Description Logic materialization. Leveraging `petgraph`'s `DiGraph` and `FixedBitSet` Breadth-First-Search (BFS) algorithms, it calculates the transitive closures of `rdfs:subClassOf`, `rdfs:domain`, `rdfs:range`, and `owl:equivalentClass` safely and instantly.
4. **Export Layer (BigQuery Exporter)**:
   The materialized graph is structured via in-memory SPARQL queries (via `oxrdflib`) and batched into BigQuery using automated JSON-L schema detection.
...
### `POST /ingest`
Ingests an ontology, processes it asynchronously, and loads it into BigQuery.

**Form Data:**
* `file`: The raw ontology file (e.g. `.owl` or `.ttl`).
* `format`: The parsing format (must be `"xml"` or `"turtle"`).
* `project_id`: Destination Cloud Project ID.
